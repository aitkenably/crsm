...
class TestLoadConfigDefaults:
    def test_returns_default_db_path_when_no_config_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("crsm.config.DEFAULT_REPO_PATH", repo_dir)
    ...
    def test_returns_default_repo_path_when_no_config_file(self, tmp_path, monkeypatch):
        ...
        assert config.repo_path == repo_dir
...
class TestLoadConfigMissingDirectories:
    ...
    def test_raises_when_repo_directory_missing(self, tmp_path, monkeypatch):
        db_parent = tmp_path / "db"
        db_parent.mkdir()
        monkeypatch.setattr("crsm.config.DEFAULT_DB_PATH", db_parent / "default.db")
        config_file = tmp_path / "config.toml"
        ...
        with pytest.raises(ConfigError, match="Repository directory does not exist"):
            load_config(config_file)
    ...
    def test_raises_when_default_repo_missing(self, tmp_path, monkeypatch):
        db_parent = tmp_path / "db"
        db_parent.mkdir()
        monkeypatch.setattr("crsm.config.DEFAULT_DB_PATH", db_parent / "default.db")
        monkeypatch.setattr("crsm.config.DEFAULT_REPO_PATH", tmp_path / "missing" / "library")
        config_file = tmp_path / "config.toml"
        config_file.write_text("")
